...
- **In-Flight Grace Period**: The ELB keeps the existing network pipes open just long enough for active users to finish their current transactions(like uploading a file, completing a checkout, or executing a heavy database query).
...
- **Turning it off**: Setting the value to `0` completely disables draining. The moment an instance is deregistered, all current connections are immediately terminated, resulting in `502 Bad Gateway` or `504 Gateway Timeout` errors for those active users.
